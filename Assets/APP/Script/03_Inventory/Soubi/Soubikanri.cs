...
public class Soubikanri : MonoBehaviour, IInventoryManager<EquipmentData1>
{
    ...
    {
        //データベース内の全装備を取得
        var list = equipmentDatabase.GetItemLists();
        
        if (list.Count >= 3)
        {
            equipmentCounts[list[0]] = 1;
            equipmentCounts[list[1]] = 1;
            equipmentCounts[list[2]] = 1;
            equipmentCounts[list[10]] = 1;
            equipmentCounts[list[13]] = 1;
            equipmentCounts[list[16]] = 1;
            equipmentCounts[list[25]] = 1;
            equipmentCounts[list[27]] = 1;
            equipmentCounts[list[30]] = 1;
            equipmentCounts[list[33]] = 1;
            equipmentCounts[list[35]] = 1;
            equipmentCounts[list[37]] = 1;
        }
        else
        {
    ...
    */
    ...
    // 現在のカテゴリ（部位）に装備
    public/*private*/ void EquipCurrentType(EquipmentData1 eq)
    {
    ...
    private void ApplyToMiddleSlot(EquipmentData1.Equipmenttype type, EquipmentData1 eq)
    {
        ...
        if (ui.iconImage) ui.iconImage.sprite = eq.GetItemicon();
    }
    ...
    private void ApplyEmptyToMiddleSlot(EquipmentData1.Equipmenttype type)
    {
        ...
        if (ui.iconImage) ui.iconImage.sprite = null;
    }
